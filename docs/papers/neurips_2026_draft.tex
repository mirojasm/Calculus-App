...
designed to induce epistemic interdependence. Across $1{,}967$ conversations,
active-learner framing increases \CDI{} from $0.362$ to $0.613$ ($d=1.30$,
$p<0.001$) and more than doubles fully coupled collaboration ($11\%$ to $24\%$),
...
reproduces the effect, supporting a synergy between structural interdependence
...
framing. We release \CPP, \CIDI{} splits, conversation logs, scoring scripts,
...
\begin{enumerate}
    ...
    \item \textbf{Framing study} ($1{,}967$ + $832$ ablation conversations): active-learner framing shifts behavior ($d=1.30$); peer-awareness alone does not; neither component alone suffices.
...
Action, Team Organisation) \citep{pisa2015cps}. We adapt this grid to
LLM-to-LLM dialogues as the basis for \CPP{} (Section~\ref{sec:framework}).

Prior multi-agent LLM work uses role differentiation \citep{park2023generative},
debate \citep{liang2023encouraging}, and jigsaw information splitting
\citep{du2023improving}. These designs focus on task-level architecture; we
study instead how an agent's \emph{epistemic stance} affects collaborative
depth, independent of task structure.
...
from \emph{activated} interdependence---a distinction invisible to \CDI{} alone.
...
\emph{(L4) Reliability sample}: the reliability evidence is
preliminary ($n=36$); per-cell $\kappa$ estimates require $\geq 100$
conversations.
\emph{(L5) Collaborative register vs.\ reasoning}: we cannot rule out
...
stance) substantially shifts agents from expert-answering to collaborative
...
\answerYes{} Graduate-student annotators provided inter-rater reliability annotations (§4.4). Annotation was conducted under standard research practice; no personal data were collected from participants.

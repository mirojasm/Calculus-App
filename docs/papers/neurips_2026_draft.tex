...
(and symmetrically for $\AEC_B$). Three aggregate measures: \textbf{\EN{}}
($v(\{A,B\}) > \max(v(\{A\}), v(\{B\}))$, pair surpasses both solos);
\textbf{\CS{}} ($v(\{A,B\}) - \max(v(\{A\}), v(\{B\}))$, gain magnitude);
\textbf{EB} ($1 - |\AEC_A - \AEC_B|$, contribution symmetry).
...
\textbf{descriptive robustness, not formal generalization}. Effect sizes
...
\textbf{(W2)} C6's negligible gain over C2 ($d = 0.16$, $p = 0.026$,
...
\textbf{(W3)} \CDI{} and correctness are orthogonal by design
...
\textbf{(W4)} A DAG-weighted \CDI{} variant ($\CDI_w$, weighting
...
\textbf{(W5)} Bootstrap $95\%$ CI for $\Delta_\CDI$(C7$-$C2)

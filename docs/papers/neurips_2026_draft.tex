...
difficulty cells of our problem corpus. A Shapley-value decomposition
(\textbf{Agent Epistemic Contribution}, \AEC) further reveals that the
\CIDI{} split successfully creates structural interdependence ($85\%$
of problems: neither agent can solve alone), and that framing converts
this latent structure into actual collaborative success ($25\%$
epistemic necessity rate; pair correctness $3.6\times$ solo maximum).
We release the \CPP{} scoring rubric, \CIDI{} generation pipeline, and
full dataset to support reproducibility.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Agent Epistemic Contribution (\AEC)}: a Shapley-value
    decomposition that quantifies when and why collaborative success
    exceeds individual capacity, revealing that \CIDI{} splits reliably
    create structural interdependence ($85\%$ both-solo-zero) and that
    role framing activates the latent epistemic necessity this structure
    enables ($25\%$ \EN{} rate; collaborative surplus $+0.207$).
\end{enumerate}
...
\paragraph{Setup.}
For each of the $140$ filtered problems we run each agent in isolation
with only its own \CIDI{} packet, using GPT-4.1-mini at temperature $0$.
Three value functions are defined (each $\in \{0, 0.5, 1\}$ following a
partial-credit scale):
\[
  v(\{A\}),\quad v(\{B\}),\quad v(\{A,B\}) = \text{best C7 replication correctness}.
\]
Shapley values for $N=2$ are:
\[
...
Necessity)}: $v(\{A,B\}) > \max(v(\{A\}), v(\{B\}))$; \textbf{\CS{}
(Collaborative Surplus)}: $v(\{A,B\}) - \max(v(\{A\}), v(\{B\}))$;
\textbf{EB (Epistemic Balance)}: $1 - |\AEC_A - \AEC_B|$.
...
The \AEC{} analysis clarifies \emph{why} framing matters. All 140
problems have structural interdependence embedded by construction
(both-solo-zero in $85\%$ of cases). The critical gap is between
structural interdependence---which the \CIDI{} split guarantees---and
epistemic necessity---which requires that the pair actually exploits the
complementary structure through coordinated reasoning. Under C2 (no
framing), agents receive the split but default to their expert-assistant
mode: they share information without genuinely negotiating a joint model,
so the $85\%$ structural interdependence converts to only $25\%$ actual
epistemic necessity. The Phase~A activation caused by C7 framing is the
mechanism that bridges this gap: by forcing agents to first build a
shared understanding of what each packet contains and why the other's
reasoning output is needed, the student-role instruction unlocks the
collaborative potential that the \CIDI{} split encodes.
...
\paragraph{Structural vs.\ activated interdependence.}
The \AEC{} decomposition reveals a two-layer structure in collaborative
success. The \CIDI{} split provides the first layer: structural
interdependence, operationalised as both-solo-zero ($85\%$ of problems).
The second layer---epistemic necessity, where the pair actually converts
structural interdependence into a superior joint outcome---depends on
how agents process the split. \EN{} reaches $25\%$ under C7, which is
the relevant operating condition; the \CS{} mean of $+0.207$ indicates
that when the pair crosses the epistemic necessity threshold, the
collaborative surplus is substantial. Crucially, \AEC{} reframes the
\emph{design problem} for multi-agent systems: creating structural
interdependence (via task design or information splitting) is achievable
but insufficient; activating that structure requires the right epistemic
framing. This mirrors results from human collaborative learning, where
jigsaw assignments improve outcomes only when participants actually
engage in deep knowledge integration rather than surface exchange
\citep{barron2003conditions}.

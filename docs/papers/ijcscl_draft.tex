...
Collaborative mathematical problem solving requires more than
distributing information across participants --- it demands that no
agent can formulate a solvable approach without engaging with another's
knowledge. We distinguish \emph{epistemic splits}, where information
distribution creates genuine reasoning interdependence, from \emph{data
splits}, which merely partition facts. Drawing on the PISA 2015
Collaborative Problem Solving framework, we introduce the Collaborative
Phase Profile (CPP), a formal binary vector over twelve cognitive cells
(four PISA processes × three competencies), and the CPS Depth Index
(CDI), a scalar measure of collaborative engagement depth. We propose a
2×2 factorial experimental design with five conditions: C1 (baseline),
C2 (CIDI-directed split, no facilitation), C3 (constitutional split, no
facilitation), C4 (CIDI split + joint accountability), and C5
(constitutional split + joint accountability). We additionally introduce
Collaborative Yield (CY), a composite metric coupling process quality
(CDI) with outcome correctness grounded in productive failure theory,
and the Minimal Framing Principle, which isolates the effect of task
structure from collaborative instruction. A four-iteration pilot study
(n=4 problems × 5 conditions) validates the framework, diagnoses three
generations of methodological failures (CDI inflation via explicit CPS
instruction, CDI collapse via goal-anchor data leak, and incomplete
consensus via external scaffolding), and establishes that collaboration
must emerge from task-role chain structure, not from prescribed
knowledge asymmetries. Implications for the design of collaborative
mathematical activities are discussed.
...
p.~134). The framework decomposes CPS into a 4 × 3 matrix: four
...
significant for that problem. A data split produces r\_pb ≈ 0 because
CDI and correctness are statistically independent --- any agent can
stumble toward the answer without genuine collaboration.

\textbf{Design example} (math\_00121, sec θ + tan θ = 22/7): An
epistemic split assigns Agent 1 the given equation and the role of
deriving sin θ and cos θ; Agent 2 receives the identity csc θ + cot θ =
(1 + cos θ)/sin θ and the task of computing m + n from the reduced
...
The Collaborative Phase Profile (CPP) is a binary vector \textbf{CPP ∈
...
\emph{{[}Figure 2 here: CIDI pipeline flow{]}}
...
\subsubsection{3.3 Experimental Design: 2×2 Factorial +
...
C2 & CIDI / No-JA & CIDI M1-M5 (task-chain) & None & Epistemic split
theory \\
C3 & Constitutional / No-JA & 36-check constitutional critic & None &
Constitutional AI critic cycle \\
C4 & CIDI / JA & CIDI M1-M5 (task-chain) & Joint accountability &
Roschelle (1992), Szewkis cond. 4 \\
C5 & Constitutional / JA & Constitutional critic & Joint accountability
& Full combination \\
\end{longtable}
}

The 2×2 structure (split type × accountability) allows additive and
interaction effects to be estimated: - C2 vs C4: Does joint
accountability improve CY when split quality is held constant (CIDI)? -
C3 vs C5: Same question with constitutional split quality. - C2 vs C3
(and C4 vs C5): Does split generation method matter when accountability
is held constant?

C1 serves as an uncontrolled reference (corpus split, no
accountability).
...
\subsubsection{3.4 Collaborative Instruction Design: The Minimal Framing
...
cannot solve this alone'', ``YOUR PRIVATE INFORMATION''. CDI mean ≈
0.65, but CDI ⊥ correctness (r = −0.015), Phase D activation was 100\%
...
collaborative context and exchange permission). CDI collapsed to ≈ 0.13.
...
goal-anchor without needing their assigned packet. C2 CDI = 0.000
despite 19 collaborative turns and a correct answer.
...
\subsubsection{3.5 LLM Agent Simulation}\label{llm-agent-simulation}

Each conversation involves n=2 agents alternating turns for a maximum of
max\_turns=8 turns (configurable). A goal-anchor message opens the
...
(previously used in legacy C4/C5) was removed after diagnosing two
...
\subsubsection{3.6 Metrics}\label{metrics}

\textbf{CDI (CPS Depth Index)}: Primary process metric. Σ active CPP
cells / 12.
...
where \(\delta_{coupling} = +1\) if CDI ≥ 0.5 AND correct (process
...
conversation: - \textbf{COUPLING} (CDI ≥ 0.5, correct): deep
collaboration produced correct answer --- target state -
\textbf{PRODUCTIVE FAILURE} (CDI ≥ 0.5, incorrect): deep collaboration
...
\subsubsection{3.6b Dynamic Answer
...
\subsubsection{3.7 Pilot Problems}\label{pilot-problems}
...
math\_00121 & Trigonometric identities (sec/tan → csc/cot, m+n) & 44 &
Yes \\
...
applied for C2-C5) because the CIDI pipeline generates a data split for
this problem. The override is documented and noted in results.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{4. Pilot Study Results}\label{pilot-study-results}

\emph{{[}Results from pilot v4 to be inserted here{]}}

\subsubsection{4.1 Methodological Development: Four
Iterations}\label{methodological-development-four-iterations}

Table 1 summarizes the four pilot iterations as a design-iteration
sequence:
...
\bottomrule\noalign{}
\endlastfoot
v1 & Initial system & \textasciitilde0.28 & n/a (bugs) & json\_mode and
selection bugs \\
v2 & Explicit CPS instruction & \textasciitilde0.65 & r=−0.015 & CDI
inflation; Phase D formulism \\
v3 & Minimal framing & \textasciitilde0.13 & --- & Root bug:
goal\_anchor data leak \\
v4 & goal\_anchor fix + task-chain M4 + joint accountability &
\emph{pending} & \emph{pending} & First clean measurement \\
\end{longtable}
}

\emph{{[}Figure 5: CDI distribution per iteration --- expected monotonic
improvement in v4{]}}

\subsubsection{4.2 Quantitative Results (v4b
pilot)}\label{quantitative-results-v4b-pilot}

\paragraph{4.2.1 CDI by condition}\label{cdi-by-condition}
...
C2 (CIDI task-chain) & \textbf{0.188} & \textbf{0.328} & 4 \\
...
The ranking C2 \textgreater{} C4 \textgreater{} C3 \textgreater{} C5 in
mean CDI is the primary empirical finding. Notably, C5 produces mean CDI
= 0.000 --- lower than the C1 baseline --- while CY remains elevated
(0.262) due to correctness on trivial problems.

\paragraph{4.2.2 Per-problem CDI
breakdown}\label{per-problem-cdi-breakdown}
...
\bottomrule\noalign{}
\endlastfoot
math\_00050 & Divisibility & 0.083 & 0.000 & 0.000 & 0.000 & 0.000 \\
math\_00128 & Factorials & 0.083 & 0.167 & 0.000 & 0.167 & 0.000 \\
math\_00121 & Trigonometry† & 0.083 & \textbf{0.583} & 0.333 & 0.333 &
0.000 \\
math\_00014 & Quadratic & 0.083 & 0.000 & 0.000 & 0.000 & 0.000 \\
\end{longtable}
}

† Split override applied (task-chain design: A1 derives sin θ/cos θ; A2
computes csc θ + cot θ).

Three of four problems (math\_00050, math\_00128, math\_00014) produce
CDI = 0.000 across all non-baseline conditions. These are
computationally accessible problems: any single LLM agent can compute
all sub-steps independently, rendering the split irrelevant. This
constitutes empirical validation of the \textbf{epistemic complexity
requirement}: task-chain design produces elevated CDI only for problems
with genuinely sequential computational dependencies.

\paragraph{4.2.3 Quadrant distribution}\label{quadrant-distribution}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Condition & COUPLING & PROD\_FAIL & TRIVIAL & COLLAPSE \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
C2 produces the pilot's only PROD\_FAIL instance (math\_00121 × C2: CDI
= 0.583, answer incorrect). All other non-C1 conditions for the
epistemic problem (C3, C4, C5) produce COLLAPSE (CDI \textless{} 0.5,
incorrect).

\paragraph{4.2.4 Joint accountability effect on
CDI}\label{joint-accountability-effect-on-cdi}

A within-problem comparison for math\_00121 reveals a \emph{negative}
...
Adding joint accountability to the same CIDI task-chain split reduces
CDI by 0.25 for the epistemic problem. The proposed mechanism: the
phrase ``Both partners must \textbf{independently} state the same final
answer'' creates a confound with the task-chain design --- Agent 2
interprets ``independently'' as license to compute the answer without
waiting for Agent 1's required intermediate output (sin θ, cos θ). This
conflicts directly with the ``Needs from partner'' field in the packet.
The finding suggests a revision to the joint accountability wording:
``Both partners must confirm agreement on the same final answer''
(removing ``independently'') to preserve chain execution while
maintaining the convergence criterion.

\subsubsection{4.3 Qualitative Analysis}\label{qualitative-analysis}

\paragraph{4.3.1 COUPLING case: task-chain collaboration in
v4}\label{coupling-case-task-chain-collaboration-in-v4}

\emph{{[}Conversation excerpt --- what genuine task-chain coupling looks
like: Agent 1 computes intermediate, Agent 2 requests it, Agent 2
combines to final answer{]}} \emph{{[}Compare: v2 version of same
problem (scripted Phase A) vs v4 (emergent Phase A){]}}

\paragraph{4.3.2 PRODUCTIVE FAILURE case: math\_00121 × C2
(v4b)}\label{productive-failure-case-math_00121-c2-v4b}

The task-chain override for math\_00121 produced CDI = 0.583 (CPP-DEEP)
with 20 turns --- the highest CDI observed in any pilot run. The chain
executed correctly: - Agent 1 (Turn 1): given sec θ + tan θ = 22/7,
derived sin θ = 435/533 and cos θ = 308/533 using the Pythagorean
identity. Shared these values immediately. - Agent 2 (Turn 2): received
sin θ and cos θ, correctly applied csc θ + cot θ = (1 + cos θ)/sin θ =
841/435. - Error at final step: Agent 2 stated ``this fraction is
already reduced'', computing m + n = 841 + 435 = 1276. In fact gcd(841,
435) = 29, giving 29/15 and m + n = 44.

This is a canonical PROD\_FAIL instance: deep collaboration (CDI =
0.583), correct chain execution, mathematical error in the final
simplification step. The collaborative process was entirely sound; the
failure is a computation capability limitation. Under Kapur's productive
failure framework, this conversation represents maximal collaborative
engagement within the ZPD --- the most educationally valuable quadrant.

\emph{{[}Excerpt: Turn 1 Agent 1 → Turn 2 Agent 2 --- functional chain
with correct intermediaries{]}}

\paragraph{4.3.3 Task-chain vs knowledge-restriction
comparison}\label{task-chain-vs-knowledge-restriction-comparison}

\emph{{[}Analysis: same problem split two ways --- ``You do NOT know X''
(v3) vs ``Task: compute X from Y'' (v4){]}} \emph{{[}Coding: Phase A
emergence, number of turns before first substantive math exchange, CDI
per coding{]}}

\paragraph{4.3.4 Joint accountability effect on Phase
D}\label{joint-accountability-effect-on-phase-d}

\emph{{[}Comparison: C2 vs C4 (same CIDI split) --- does joint
accountability change answer convergence rate?{]}} \emph{{[}Comparison:
C3 vs C5 --- same with constitutional split{]}}

\paragraph{4.3.5 Data leak pathology (v3 root-cause
illustration)}\label{data-leak-pathology-v3-root-cause-illustration}

\emph{{[}C2 v3 case: CDI=0.000 despite 19 turns and correct answer ---
full problem in goal\_anchor, agents solved independently from turn 1,
split packets ignored{]}}
...
valuable even without immediate success (Kapur, 2012)\ldots{}
...
information should each participant keep private?''\ldots{}
...
\item
  \textbf{Pilot scale}: n=4 problems × 5 conditions = 20 conversations
...
This paper has introduced the CPP framework, the CDI metric, the CY
composite, and the minimal framing principle as theoretical and
methodological contributions to the design and study of AI-generated
collaborative mathematical tasks. The pilot study demonstrates that
genuine CPS measurement requires epistemic splits, not data splits, and
that collaborative behavior emerges from task structure when framing is
minimal.

Future work includes: - Validation with human learners at defined
curriculum levels (applying domain restriction, not persona, to the
agent if used as a peer) - Scale-up to n=30 problems across multiple
mathematical domains - Extension to n=3 and n=4 agent groups to test
phase dynamics at higher n - Automated epistemic fidelity scoring (r\_pb
estimation from M5 output) - Curriculum mapping: applying the CPP
analyzer to identify epistemic split opportunities across K-12
mathematics
...
\emph{{[}To be completed --- key sources:{]}} - OECD (2017). PISA 2015
Results (Volume V): Collaborative Problem Solving. - Greiff, S. et
al.~(2014). Domain-general problem solving skills and education. -
Kapur, M. (2012). Productive failure in learning the concept of
variance. - Chi, M. T. H., \& Wylie, R. (2014). The ICAP framework. -
Dillenbourg, P. (1999). What do you mean by collaborative learning? -
Szewkis, E. et al.~(2011). Collaboration within large groups in the
classroom. - Kirschner, F. et al.~(2018). From cognitive load theory to
collaborative cognitive load theory. - Webb, N. M. (1991). Task-related
verbal interaction and mathematics learning. - Sfard, A. (1998). On two
metaphors for learning. - Vygotsky, L. S. (1978). Mind in society.
